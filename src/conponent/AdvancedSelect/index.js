...
class AdvancedSelect extends PureComponent {
  ...
  constructor(props) {
    ...
    this.defaultConfig = {
      ...
      TASK_DEPARTMENT : {
        ...
        componentConfig : {
          onSearch : (v) => this.handleSearch(v, 'departmentPy'),
          showSearch : true,
          filterOption : false,
        },
      },
      ...
      USER_FUZZYSEARCH : {
        ...
        componentConfig : {
          onSearch : (v) => this.handleSearch(v, 'nickname'),
          showSearch : true,
          filterOption : false,
        },
      },
      DEP_FUZZYSEARCH:{
        ...
        componentConfig : {
          onSearch : (v) => this.handleSearch(v, 'depPy'),
          showSearch : true,
          filterOption : false,
        },
      },
  ...
  handleSearch = (value, searchType) => {
    ...
    const res = dataSource.filter((v) => {
      return v[searchType] && v[searchType].indexOf(value) != -1;
    });
      ...
      return optionData.map((item) => {
        return (
          <Option
            ...
            value={item[fieldConfig.value]?.toString()}
          >
